...
- True reporter stories now have a separately labelled CMS editorial-correction path. The database-owned `SECURITY DEFINER` RPC requires an active editor/admin, locks the story and latest immutable revision, checks both the expected revision and canonical `updated_at`, and validates a complete bounded editorial patch and story-owned featured media. Submitted revisions, reporter provenance/byline, and private location evidence remain untouched.
...
- Focused reporter/CMS contracts: 34/34 passed.
- Full root tests with bundled Node.js 24.19.0: website 233/233, CMS 606/606, reporter 205/205.
...
## Final privacy and recovery follow-up

- Free-form correction reasons are stored only in an append-only private correction event table with no Data API grants. Generic audit metadata contains only the private event ID, revision ID, and changed field names; the rollback verifier rejects reason or coordinate keys there.
- A failed post-submission local cleanup keeps the entire stale editor locked and exposes one accessible cleanup-and-refresh retry outside the disabled fieldset. Runtime coverage verifies failed cleanup cannot refresh or discard the recovery snapshot and a later successful retry refreshes exactly once.
