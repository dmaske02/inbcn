-- Reporter media becomes canonical only after the server has reloaded and
-- verified the immutable Cloudinary asset through the authenticated Admin API.
...
create unique index media_cloudinary_asset_id_key
on public.media ((metadata ->> 'cloudinaryAssetId'))
where nullif(metadata ->> 'cloudinaryAssetId', '') is not null;

create function public.complete_reporter_media_upload(
  p_profile_id uuid,
...
declare
  ...
  object_id text := split_part(p_public_id, '/', 6);
begin
  ...
  if p_asset_id is null or p_asset_id !~ '^[A-Za-z0-9_-]{1,255}$'
    ...
    or p_public_id is distinct from
      'inbcn/reporter/story/' || p_profile_id::text || '/' || p_story_id::text || '/' || object_id
