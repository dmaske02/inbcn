...
alter table public.live_recordings
  ...
  add constraint live_recordings_terminal_reconciliation_status_check check (
    ...
    or terminal_reconciliation_status in ('completed', 'failed')
  );

comment on column public.live_recordings.terminal_reconciliation_status is
  'DB-owned monotonic provider terminal fact from start reconciliation. Never exposed to browsers, public projections, audit metadata, or notifications.';
...
begin
  ...
  if current_recording.terminal_reconciliation_status is not null
...
begin
  ...
  set egress_id = p_egress_id,
      terminal_reconciliation_status = coalesce(terminal_reconciliation_status, p_provider_status)
  where id = current_recording.id
