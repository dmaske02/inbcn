...
test("public Homepage Builder repository is read-only and ordered", async () => {
  ...
  assert.match(boundary,/createAdminClient/u); assert.match(boundary,/homepage_configurations/u); assert.match(boundary,/homepage_sections/u); assert.match(boundary,/\.order\("position", \{ ascending: true \}\)/u);
