...
import { revalidateWebsite } from "../public-revalidation";
...
async function revalidateLiveTvLocales(locales: readonly string[]): Promise<void> {
  for (const locale of locales) revalidateLiveTv(locale);
  await revalidateWebsite("live-tv");
}
...
export async function createLiveTvAction(
  ...
  await revalidateLiveTvLocales(result.locales);
