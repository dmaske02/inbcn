...
export function LiveReviewDetail({ request, canDecide }: Readonly<{ request: LiveReviewRequest; canDecide: boolean }>) {
  ...
  return (
    <div className="space-y-6">
      ...
      {canDecide && request.status === "approved" ? <section className="rounded-md border border-border p-5 lg:p-6"><form action={terminateAction} className="space-y-4"><h2 className="text-lg font-semibold">Terminate request</h2><p className="text-sm text-muted-foreground">This changes the request workflow only. It does not delete a LiveKit room or stop a provider session.</p><label className="block text-sm font-medium" htmlFor="termination-reason">Termination reason<textarea className={inputClass} id="termination-reason" maxLength={2000} minLength={1} name="reason" required rows={4} /></label><Button disabled={terminatePending} type="submit" variant="destructive">Terminate live request</Button><StateMessage pending={terminatePending} state={terminateState} /></form></section> : null}
