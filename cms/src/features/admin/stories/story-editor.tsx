  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        {view.story?.status === "published" ? <Link className={buttonVariants({variant:"outline"})} href={`/${view.references.languages.find((item)=>item.id===view.story?.languageId)?.code ?? "en"}/${view.story.slug}`} target="_blank">View public URL</Link> : null}
      </header>
