...
export function buildHomepagePreview(locale: HomepageLocale, sections: readonly HomepageSectionDto[], references: HomepageReferenceData, now = new Date()): HomepagePreviewPayload {
  const items = sections.filter((section) => section.enabled && (!section.startsAt || new Date(section.startsAt) <= now) && (!section.endsAt || new Date(section.endsAt) > now)).toSorted((a, b) => a.position - b.position).map((section) => {
    const definition = getHomepageBlockDefinition(section.blockType);
    if (!definition || definition.renderer !== section.renderer) throw new Error(`Block ${section.blockId} has an invalid renderer.`);
    const result = definition.validate(section.configuration);
    if (!result.success) throw new Error(`Block ${section.blockId} has invalid configuration.`);
    ...
    if (section.blockType === "hero-story") {
      ...
      if (!story) throw new Error(`The story is missing for ${section.title}.`);
    ...
    if (section.blockType === "category-section") {
      ...
      if (!category) throw new Error(`The category is missing for ${section.title}.`);
      configuration.category = category;
    }
    if (section.blockType === "live-tv") {
      if (!references.liveTv) throw new Error("The Live TV configuration is missing for this locale.");
