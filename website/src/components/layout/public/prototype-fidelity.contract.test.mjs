...
test("public chrome exposes every approved editorial control and surface", async () => {
...
test("mobile drawer exposes localized Live TV first and closes after navigation", async () => {
  const source = await readFile(new URL("./editorial-shell.tsx", import.meta.url), "utf8");
  const drawerStart = source.indexOf('<nav className="editorial-drawer-links"');
  const categoriesStart = source.indexOf("{categories.map", drawerStart);
  const liveTvLink = source.indexOf('className="editorial-drawer-live"', drawerStart);
  ...
  assert.match(source.slice(liveTvLink, categoriesStart), /onClick=\{closeDrawer\}/u);
});
...
test("public chrome receives a server-formatted date instead of formatting during hydration", async () => {
  const [chrome, layout] = await Promise.all([
    readFile(new URL("./editorial-shell.tsx", import.meta.url), "utf8"),
    readFile(new URL("./public-layout.tsx", import.meta.url), "utf8"),
  ]);

  assert.match(layout, /const currentDate = new Intl\.DateTimeFormat/u);
  assert.match(layout, /<EditorialShell[\s\S]*currentDate=\{currentDate\}/u);
