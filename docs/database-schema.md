...
`supabase/migrations/20260822166000_reporter_lifecycle.sql` adds the bounded
daily reporter lifecycle boundary. Applications persist a one-time completion
reminder marker, and reporter profiles persist the exact membership expiry for
which a renewal reminder was committed. The database clock and locked current
rows own application cancellation, membership reminder/grace/expiry, and
provider-work eligibility; concurrent renewal therefore wins before a stale
